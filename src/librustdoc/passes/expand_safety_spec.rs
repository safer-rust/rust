...
//!
//! ```text
//! /// #[safety::requires(ValidPtr(dst, T, 1))]
//! ```
//!
//! will be expanded to:
//!
//! ```text
//! /// ValidPtr: pointer `{dst}` must be valid for reading and writing the `sizeof(T)* {1}` memory from it 
//! ```
//!
//! # TOML format
//!
...
///
/// ```rust,ignore
/// let def = TagDef { args: vec!["p", "T", "len"], desc: "pointer `{p}` must be valid for reading and writing the `sizeof({T})* {len}` memory from it" };
/// let values = vec!["dst", "i32", "1"];
/// let rendered = render_desc(&def, &values);
/// assert_eq!(rendered, "pointer `dst` must be valid for reading and writing the `sizeof(i32)* 1` memory from it");
/// ```
...
///
/// ```rust,ignore
/// let payload = "ValidPtr(dst, T, 1)";
/// let (tag, args) = parse_tag_call(payload).unwrap();
/// assert_eq!(tag, "ValidPtr");
/// assert_eq!(args, ["dst", "T", "1"]);
/// ```
...
///
/// ```rust,ignore
/// let payload = "ValidPtr(dst, i32, 1)";
/// let mut spec = SafetySpec { package_name: "core".to_string(), tags: FxHashMap::default() };
/// spec.tags.insert("ValidPtr".to_string(), TagDef { args: vec!["p", "T", "len"], desc: "pointer `{p}` must be valid for reading and writing the `sizeof({T})* {len}` memory from it" });
/// let expanded = expand_tag_call(payload, &spec);
/// assert_eq!(expanded, Some("ValidPtr: pointer `dst` must be valid for reading and writing the `sizeof(i32)* 1` memory from it".to_string()));
/// ```
...
///
/// ```rust,ignore
/// let s = "#[safety::requires(ValidPtr(dst, i32, 1))]";
/// let result = parse_requires_span_at_start(s);
/// assert_eq!(result, Some((42, "ValidPtr(dst, i32, 1)")));
/// ```
...
mod tests {
    ...
    fn test_render_desc() {
        let def = TagDef {
            args: vec!["p".to_string(), "T".to_string(), "len".to_string()],
            desc: "pointer `{p}` must be valid for reading and writing the `sizeof({T})* {len}` memory from it"
                .to_string(),
        };
        let values = vec!["dst".to_string(), "i32".to_string(), "1".to_string()];
        let rendered = render_desc(&def, &values);
        assert_eq!(
            rendered,
            "pointer `dst` must be valid for reading and writing the `sizeof(i32)* 1` memory from it"
        );
    }

    #[test]
    fn test_matching_paren_close() {
        let s = "(dst, T, 1)";
        assert_eq!(matching_paren_close(s, 0), Some(10));
    }
    ...
    fn test_split_top_level_args() {
        let inner = "dst, T, 1";
        let args = split_top_level_args(inner);
        assert_eq!(args, vec!["dst".to_string(), "T".to_string(), "1".to_string()]);
    }

    #[test]
    fn test_parse_tag_call() {
        let payload = "ValidPtr(dst, T, 1)";
        let (tag, args) = parse_tag_call(payload).unwrap();
        assert_eq!(tag, "ValidPtr");
        assert_eq!(args, vec!["dst".to_string(), "T".to_string(), "1".to_string()]);
    ...
    #[test]
    fn test_expand_tag_call() {
        let payload = "ValidPtr(dst, i32, 1)";
        let mut spec = SafetySpec {
        ...
        assert_eq!(
            expanded,
            Some(
                "ValidPtr: pointer `dst` must be valid for reading and writing the `sizeof(i32)* 1` memory from it"
                    .to_string()
            )
        );
    }
    ...
    #[test]
    fn test_parse_requires_span_at_start() {
        let s = "#[safety::requires(ValidPtr(dst, i32, 1))]";
        assert_eq!(parse_requires_span_at_start(s), Some((42, "ValidPtr(dst, i32, 1)")));
    }
}
